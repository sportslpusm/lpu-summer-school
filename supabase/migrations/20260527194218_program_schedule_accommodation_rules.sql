alter table public.programs
  add column if not exists schedule_type text not null default 'selectable',
  add column if not exists accommodation_mode text not null default 'none',
  add column if not exists allow_mess boolean not null default false,
  add column if not exists mess_meal_rate integer,
  add column if not exists mess_meals_per_day integer,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'programs_schedule_type_check'
      and conrelid = 'public.programs'::regclass
  ) then
    alter table public.programs
      add constraint programs_schedule_type_check
      check (schedule_type in ('selectable', 'fixed'));
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'programs_accommodation_mode_check'
      and conrelid = 'public.programs'::regclass
  ) then
    alter table public.programs
      add constraint programs_accommodation_mode_check
      check (accommodation_mode in ('none', 'optional', 'included'));
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'programs_mess_meal_rate_check'
      and conrelid = 'public.programs'::regclass
  ) then
    alter table public.programs
      add constraint programs_mess_meal_rate_check
      check (mess_meal_rate is null or mess_meal_rate >= 0);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'programs_mess_meals_per_day_check'
      and conrelid = 'public.programs'::regclass
  ) then
    alter table public.programs
      add constraint programs_mess_meals_per_day_check
      check (mess_meals_per_day is null or mess_meals_per_day between 0 and 10);
  end if;
end;
$$;

alter table public.courses
  drop constraint if exists courses_min_age_check;

alter table public.courses
  add constraint courses_min_age_check check (min_age between 0 and 70);
...
alter table public.registrations
  add constraint registrations_student_age_check
    check (student_age is null or student_age between 3 and 70),
  add constraint registrations_class_level_check
    check (class_level = any (array[
      'Class 6',
      'Class 7',
      'Class 8',
      'Class 9',
...
set
  schedule_type = case when slug = 'staff-camp' then 'fixed' else coalesce(schedule_type, 'selectable') end,
  accommodation_mode = case
    when slug = 'immersion' then 'included'
    when allow_hostel then 'optional'
    else coalesce(nullif(accommodation_mode, ''), 'none')
  end,
  allow_mess = case when allow_hostel then true else coalesce(allow_mess, false) end,
  mess_meal_rate = coalesce(mess_meal_rate, 80),
  mess_meals_per_day = coalesce(mess_meals_per_day, 3),
  included_services = case
    when slug = 'immersion' and nullif(included_services, '') is null
      then 'Stay, food, campus experiences, and guided activities are included when this package is announced.'
    else included_services
  end,
  updated_at = now();

insert into public.site_config (key, value, description, updated_at)
values
  ('mess_meal_fee', '80', 'Default mess meal price per student, per meal.', now()),
  ('mess_meals_per_day', '3', 'Default number of mess meals charged per student per day.', now())
...
begin
  ...
  if v_student_age < 3 or v_student_age > 70 then
    raise exception 'Student age must be between 3 and 70';
  end if;
