alter table public.programs
  add column if not exists date_display_mode text not null default 'date_range';

alter table public.programs
  drop constraint if exists programs_date_display_mode_check;

alter table public.programs
  add constraint programs_date_display_mode_check
    check (date_display_mode in ('date_range', 'self_paced', 'to_be_announced'));

update public.programs
set date_display_mode = case
    when slug = 'online' then 'self_paced'
    when start_date is null and end_date is null and coalesce(dates_label, '') ilike '%self%paced%' then 'self_paced'
    when start_date is null and end_date is null then 'to_be_announced'
    else 'date_range'
  end,
  dates_label = case
    when slug = 'online' then 'Self-paced'
    when start_date is null and end_date is null and coalesce(dates_label, '') ilike '%self%paced%' then 'Self-paced'
    when start_date is null and end_date is null then 'Date to be announced'
    else dates_label
  end,
  duration = case
    when slug = 'online' then 'Self-paced'
    when start_date is null and end_date is null and coalesce(dates_label, '') ilike '%self%paced%' then 'Self-paced'
    when start_date is null and end_date is null then 'To be announced'
    else duration
  end,
  start_date = case
    when slug = 'online' then null
    when start_date is null and end_date is null and coalesce(dates_label, '') ilike '%self%paced%' then null
    else start_date
  end,
  end_date = case
    when slug = 'online' then null
  ...
  updated_at = now()
where date_display_mode is distinct from case
    when slug = 'online' then 'self_paced'
    when start_date is null and end_date is null and coalesce(dates_label, '') ilike '%self%paced%' then 'self_paced'
    when start_date is null and end_date is null then 'to_be_announced'
    else 'date_range'
