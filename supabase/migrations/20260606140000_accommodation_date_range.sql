...
declare
  ...
  v_date_display_mode text := 'date_range';
  ...
  v_hostel_start date := case when payload->>'hostel_start_date' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then (payload->>'hostel_start_date')::date else null end;
  v_hostel_end date := case when payload->>'hostel_end_date' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then (payload->>'hostel_end_date')::date else null end;
...
begin
  ...
  v_date_display_mode := coalesce(v_program.date_display_mode, 'date_range');

  if not v_program.registration_enabled
    or v_program.fee_status <> 'ready'
    or v_program.fee_mode = 'to_be_announced'
    or v_date_display_mode = 'to_be_announced'
    or (v_date_display_mode = 'date_range' and (v_program.start_date is null or v_program.end_date is null)) then
    raise exception 'Registration for % will open once schedule timing, schedule, and fee are announced', v_program.name;
  end if;
  ...
  -- A dated program that has reached its start date is in progress; registration is closed.
  if v_date_display_mode = 'date_range' and v_program.start_date is not null
     and now() >= (v_program.start_date::timestamp at time zone 'Asia/Kolkata') then
    raise exception '% has already started, so registration is closed', v_program.name;
  end if;
  ...
  if v_hostel_option not in ('none', 'included') then
    if v_hostel_start is not null and v_hostel_end is not null
       and v_program.start_date is not null and v_program.end_date is not null then
      if v_hostel_start > v_hostel_end then
        raise exception 'Stay check-out date cannot be before the check-in date';
      end if;
      if v_hostel_start < (v_program.start_date - 1) or v_hostel_end > (v_program.end_date + 1) then
        raise exception 'Stay dates must be between % and %',
          to_char(v_program.start_date - 1, 'DD Mon YYYY'), to_char(v_program.end_date + 1, 'DD Mon YYYY');
      end if;
      v_hostel_days := greatest((v_hostel_end - v_hostel_start) + 1, 1);
    else
      v_hostel_days := case
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'dates_label', case when v_date_display_mode = 'self_paced' then 'Self-paced' when v_date_display_mode = 'to_be_announced' then 'Date to be announced' else v_program.dates_label end,
      'date_display_mode', v_date_display_mode,
      'mode', v_program.mode,
